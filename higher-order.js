...
let orderTotals = orders.map(function(elem) {
  let tax = elem.price * elem.tax
  let total = tax + elem.price
  return total
})

 // -----WHY DOESNT THIS CODE WROK?----- //

// let orderTotals = orders.map(elem => {
//   let actualTax = elem.price * elem.tax
//   elem.price += actualTax
//   return elem.price
// })
...
let justBob = purchases.filter(function(elem) {
  if (elem.owner === "Bob") {
    return elem.owner
  }
}); 
...
justBob.forEach(function(element){
